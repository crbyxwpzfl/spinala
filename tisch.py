...
async def move():  # moves tisch
    while not math.isclose( d.get('position', 1111) , d['dsrd'] , abs_tol=5):  # TODO calculate tolerance including round()
        ...
        print(d['lineup'], end=d['lineclear']);    print(d['lineup'], end=d['lineclear']);    print("this server respondes to 'Set/Get whatever whatever int' and drives tisch to int. int can be 'R+\{1}'");    print(f"rawpos-{d['rawpos']} transpos-{d['transpos']} desired-{d['dsrd']} ")  # pretty print
        if d['transpos'] < d['dsrd'] and not d.get('moving') == "up": d['moving'] = "up";    d['drivepin'](eval(d['pindw']), False);    d['drivepin'](eval(d['pinup']), True)
        if d['transpos'] > d['dsrd'] and not d.get('moving') == "dw": d['moving'] = "dw";    d['drivepin'](eval(d['pinup']), False);    d['drivepin'](eval(d['pindw']), True)
